...
<div class="inright" style="background-color: #fff;">
	...
	<div class="mbody">
		<h1 class="title1"><h1 class="text-center"><?php echo $news_row['title'] ?></h1></h1>
		<div class="title2">来源：本站　　　发布日期：<?php echo $news_row['intime'] ?>　　　已有 <?php echo $news_row['hits'] ?> 人浏览过此信息</div>
		<div class="newsbody">
			<h2><?php echo $news_row['content'] ?></h2>
		</div>
		...
		<div class="newsmore">
				<?php
					...
					if($row=mysqli_fetch_assoc($rs)){
						echo '<h3>上一条：<a href="content.php?id='.$row['id'].'">'.$row['title'].'</a> </h3>';
					}else{
		...
		<div class="newsmore">
				<?php
					...
					$rs=mysqli_query($conn,$sql);
					if($row=mysqli_fetch_assoc($rs)){
						echo '<h3>下一条：<a href="content.php?id='.$row['id'].'">'.$row['title'].'</a> </h3>';
					}else{
